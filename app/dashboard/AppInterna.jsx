...
function Obras({ obras, setObras, lics, detailId, setDetailId, requireAuth, cfg, apiKey }) {
    ...
    async function handleArch(e) {
        ...
        for (const f of Array.from(e.target.files)) {
            const dataUrl = await toDataUrl(f);
            const archId = uid();
            // Guardar en key separada (no en el JSON de obras — es muy pesado)
            await storage.set(SP+'archdata_'+archId, dataUrl).catch(()=>{});
            try { localStorage.setItem(SP+'archdata_'+archId, dataUrl); } catch {}
            const archsMeta = [...(detail.archivos||[]), {
                id: archId, archKey: SP+'archdata_'+archId,
                nombre: f.name, ext: f.name.split('.').pop().toUpperCase(),
                fecha: new Date().toLocaleDateString('es-AR')
            }];
            upd(detail.id, { archivos: archsMeta });
        }
        e.target.value = '';
    }
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "14px 18px", paddingBottom: 80 }}>
                    ...
                    {tab === "archivos" && (<div>
                        ...
                        {detail.archivos.map(f => (<div key={f.id} style={{ display: "flex", alignItems: "center", gap: 10, background: T.card, border: `1px solid ${T.border}`, borderRadius: T.rsm, padding: "11px 13px", marginBottom: 7 }}>
                            ...
                            <button onClick={() => upd(detail.id, { archivos: detail.archivos.filter(x => x.id !== f.id) })} style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, width: 30, height: 30, fontSize: 12, color: '#EF4444', cursor: 'pointer' }}>✕</button>
                        </div>))}
...
function Mas({ setView, setUser, user, cfg, setCfg, apiKey, setApiKey, obras, setObras, lics, setLics, empresa, onCambiarEmpresa, personal, setPersonal }) {
    ...
    async function handleLogoUpload(key, file) {
        ...
        // Guardar logo en key separada (no dentro del cfg)
        const logoKey = SP + 'cfg_logo_' + key.replace('logo', '').replace('Belfast','b').replace('Central','c').replace('Asistente','a').replace('AA2000','v').toLowerCase().slice(0,1);
        storage.set(logoKey, compressed).catch(()=>{});
        try { localStorage.setItem(logoKey, compressed); } catch {}
