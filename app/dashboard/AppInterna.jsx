    ...
    return (<div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        <div style={{ background: T.navy, padding: "16px 18px 20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginTop: 16 }}>
                {[{ l: t(cfg, 'dash_obras_activas'), v: obras.filter(o => o.estado === "curso").length, c: "#34D399" }, { l: 'Total obras', v: obras.length, c: "#60A5FA" }, { l: t(cfg, 'dash_alertas'), v: alerts.length, c: "#FBBF24" }, { l: t(cfg, 'dash_personal'), v: personal.length, c: "#A78BFA" }].map(k => (
...
Al final incluí: [[ACTION:{"tipo":"subir_minuta","obraId":"${obraReunion}","titulo":"Minuta reunión ${new Date().toLocaleDateString('es-AR')}"}]]`;
    ...
    return (<div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: T.bg }}>
        ...
        <div style={{ padding: "8px 10px", background: T.card, borderTop: '1px solid ' + T.border, display: "flex", gap: 6, alignItems: "center", position: "fixed", bottom: 72, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, zIndex: 99 }}>
            ...
            {/* Indicador GPS — se activa solo, muestra estado */}
            <div title={gpsPos ? `📍 ${gpsPos.lat.toFixed(4)}, ${gpsPos.lng.toFixed(4)}` : 'GPS no disponible'}
                style={{ width: 36, height: 36, borderRadius: "50%", background: gpsPos ? '#ECFDF5' : '#FEF2F2', border: `1px solid ${gpsPos ? '#10B981' : '#FECACA'}`, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>
                {gpsPos ? '📍' : '📵'}
            </div>
